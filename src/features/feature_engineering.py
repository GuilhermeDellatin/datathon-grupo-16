...
def validate_raw_data(df: pd.DataFrame) -> pd.DataFrame:
    """Valida schema dos dados brutos via Pandera.

    Args:
        df: DataFrame com dados OHLCV.

    Returns:
        DataFrame validado (Pandera retorna o próprio DF se válido).

    Raises:
        pandera.errors.SchemaError: Se as colunas/tipos não baterem com
            ``RAW_SCHEMA``.
        AssertionError: Se o DataFrame estiver vazio.
    """
    logger.info("Validando schema dos dados brutos (Pandera)...")
...
def validate_feature_data(df: pd.DataFrame) -> pd.DataFrame:
    """Valida schema das features pós-engenharia via Pandera.
    ...
    Raises:
        pandera.errors.SchemaError: Se as features não baterem com
            ``FEATURE_SCHEMA``.
    """
    logger.info("Validando schema das features (Pandera)...")
