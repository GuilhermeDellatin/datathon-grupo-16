...
API_IMAGE = os.environ.get("API_IMAGE", "datathon-lstm-stocks-api:latest")
...
def ab_test_comparison(**context):
    ...
    improvement_pct = (ab_results["challenger_accuracy"] - ab_results["champion_accuracy"]) / ab_results["champion_accuracy"]
...
def quality_gate_check(**context):
    ...
    model_metrics = context["task_instance"].xcom_pull(
        task_ids="validate_model", key="model_metrics"
    ) or {}
    
    ab_results = context["task_instance"].xcom_pull(
        task_ids="ab_test", key="ab_test_results"
    ) or {}
    ...
    failed_gates = [gate for gate, passed in quality_gates.items() if not passed and gate != "all_gates_passed"]
...
def generate_report(**context):
    ...
    # Salvar relatório
    report_path = os.path.join(METRICS_DIR, f"mlops_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
with DAG(
    ...
    collect_data = DockerOperator(
        ...
        network_mode="host",
        mounts=[Mount(source=PROJECT_ROOT, target="/app", type="bind")],
        environment={
            "PYTHONUNBUFFERED": "1",
    ...
    [generate_pipeline_report >> success_notification, generate_pipeline_report >> failure_notification]
